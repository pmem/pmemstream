...
#include "../tests/common/unittest.hpp"
#include "measure.hpp"
...
class Config {
 ...
 public:
	...
	int parse_arguments(int argc, char *argv[])
	{
		...
		int ch;
		while ((ch = getopt_long(argc, argv, "p:x:b:r:c:s:i:h", long_options, NULL)) != -1) {
			switch (ch) {
				case 'p':
	...
	std::string usage()
	{
		return "Usage:" + app_name + "[OPTION]...\n" + "Pmemstream benchmark for append\n" +
			...
			"\n\t--iterations [iterations] Number of iterations. More iterations gives more robust statistical data, but takes more time";
	}
};
...
std::ostream &operator<<(std::ostream &out, Config const &cfg)
{
	...
	out << "ement_size: " << cfg.element_size << ", ";
...
int main(int argc, char *argv[])
{
	...
	auto data = benchmark::generate_data(bytes_to_generate);
	...
	auto results = benchmark::measure<std::chrono::nanoseconds>(config.iterations, [&] {
		for (auto it = data.begin(); it != data.end(); std::advance(it, config.element_size)) {
			pmemstream_append(stream.get(), region, NULL, &it, config.element_size, NULL);
		}
	});
	...
	std::cout << "\tstdandard deviation " << std_dev << "ns" << std::endl;
}
