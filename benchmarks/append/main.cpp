...
class config {
 ...
 public:
	...
	std::string path;
	size_t size = (PMEMLOG_MIN_POOL > TEST_DEFAULT_STREAM_SIZE ? PMEMLOG_MIN_POOL : TEST_DEFAULT_STREAM_SIZE);
...
class pmemlog_workload : public workload_base {
 ...
 public:
	...
	{
		for (auto it = data.begin(); it != data.end(); std::advance(it, cfg.element_size)) {
			if (pmemlog_append(plp, &it, cfg.element_size) < 0) {
	...
	virtual int initialize() override
	{
		...
		plp = pmemlog_create(path, cfg.size, 0666);
		if (plp == NULL)
...
int main(int argc, char *argv[])
{
	...
	workload_base *workload = nullptr;

	if (cfg.engine == "pmemlog") {
		workload = new pmemlog_workload(cfg);
	} else if (cfg.engine == "pmemstream") {
		workload = new pmemstream_workload(cfg);
	}
	...
	auto results =
		benchmark::measure<std::chrono::nanoseconds>(cfg.iterations, [workload] { workload->perform(); });
