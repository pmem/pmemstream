...
#define SLIST_FOREACH(type, runtime, list, it, field)                                                                 \
	for ((it) = (list)->head; (it) != UINT64_MAX;                                                                \
	     (it) = ((type *)(pmemstream_offset_to_ptr(runtime, it)))->field)

#define	SLIST_NEXT(runtime, it, field) (it = (type *)(pmemstream_offset_to_ptr(runtime, it)))->field)
