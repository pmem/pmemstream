				   ...
				   size_t size)
{
	...
	assert(span_get_type(span_offset_to_span_ptr(runtime, allocator_region_free)) == SPAN_REGION);
	assert(span_get_size(span_offset_to_span_ptr(runtime, allocator_region_free)) == size);
