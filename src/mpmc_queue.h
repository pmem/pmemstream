 ...
 *
 * Producers call 'mpmc_queue_acquire' to get offset which specifies some range [offset, offset + size)
 * such that no other producer gets the same or overlapping range. After producer is ready to
 * make it's range available to consume (e.g. wants to publish it's data) he calls 'mpmc_queue_produce'.
 *
 ...
 *
 * Returns MPMC_QUEUE_OFFSET_MAX on error. This call can fail if returned offset would be
