				   ...
				   struct pmemstream_entry *user_entry)
{
	...
	if (initialized) {
		...
		uint64_t commited_offset =
			__atomic_load_n(&iterator->region_runtime->commited_offset, __ATOMIC_ACQUIRE);
		if (iterator->offset >= commited_offset) {
			return -1;
		}

		/* commited_offset (and hence iterator->offset) should not be bigger than end of region offset. */
		assert(commited_offset <= region_end_offset);
