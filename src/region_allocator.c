...
void remove_head(struct allocator_header *alloc_header, struct singly_linked_list *list)
{
...
uint64_t region_allocator_allocate(struct allocator_header *alloc_header, uint64_t size)
{
	return EMPTY_OBJ;
}
...
void region_allocator_release(struct allocator_header *alloc_header, uint64_t offset)
{
	...
	push_back(alloc_header, &alloc_header->free_list, offset);
	span_b->size_and_type &= SPAN_EXTRA_MASK;
