...
static bool id_manager_critnib_get_lower_key(critnib *ids, uint64_t key, uint64_t *next_key)
{
	void *value = NULL;
...
static bool id_manager_critnib_get_higher_key(critnib *ids, uint64_t key, uint64_t *next_key)
{
	void *value = NULL;
...
{
	return id_manager_critnib_get_higher_key(ids, 0, key);
}

static bool id_manager_critnib_get_max_key(critnib *ids, uint64_t *key)
{
	return id_manager_critnib_get_lower_key(ids, UINT64_MAX, key);
}
...
static void id_manager_do_compaction(struct id_manager *manager)
{
	...
	while (1) {
		...
		uint64_t next_id;
		bool found = id_manager_critnib_get_lower_key(manager->ids, id, &next_id);
		if (!found) {
