		       ...
		       struct pmemstream_entry reserved_entry)
{
	...
	/* XXX: will this work with async append...? */
...
{
	printf("pmemstream_async_publish_impl!\n");
...
struct pmemstream_async_append_fut pmemstream_async_append(struct vdm *vdm, struct pmemstream *stream,
							   struct pmemstream_region region,
							   struct pmemstream_region_runtime *region_runtime, void *data,
							   size_t size)
{
	...
	if (!region_runtime) {
		int ret = pmemstream_region_runtime_initialize(stream, region, &region_runtime);
		if (ret) {
			/* XXX: can/should we return an "empty" future...? */
	...
	/* now, we chain tasks needed to complete an append */
	FUTURE_CHAIN_ENTRY_INIT(&future.data.memcpy, vdm_memcpy(vdm, reserved_dest, data, size, PMEM2_F_MEM_NOFLUSH),
				NULL, NULL);
	/* XXX: use publish_to_append_map */
