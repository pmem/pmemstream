...
{
	uint64_t count = 0;
	uint64_t aligned_size = ALIGN_DOWN(size, sizeof(uint64_t));
	for (size_t i = 0; i < aligned_size / sizeof(uint64_t); i++) {
		count += util_popcount64(data[i]);
	}

	uint8_t *d = (uint8_t*) &data[aligned_size];
	for (size_t i = 0; i < size - aligned_size; i++)
