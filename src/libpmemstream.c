...
int pmemstream_from_map(struct pmemstream **stream, size_t block_size, struct pmem2_map *map)
{
	...
	s->region_runtime_map = region_runtime_map_new();
	if (!s->region_runtime_map) {
		goto err;
	}
...
void pmemstream_delete(struct pmemstream **stream)
{
	...
	region_runtime_map_destroy(s->region_runtime_map);
	free(s);
	*stream = NULL;
}
...
int pmemstream_region_free(struct pmemstream *stream, struct pmemstream_region region)
{
	...
	region_runtime_map_remove(stream->region_runtime_map, region);
...
{
	return region_runtime_map_get_or_create(stream->region_runtime_map, region, region_runtime);
}
