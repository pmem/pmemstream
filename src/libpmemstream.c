							      ...
							      struct future_notifier *notifier)
{
	...
	if (committed_timestamp != data->first_timestamp) {
		 ...
		 * committed_timestamp. */
		void *value = critnib_get(data->stream->ready_timestamps, committed_timestamp);
		while (value && committed_timestamp + (uint64_t)value <= data->last_timestamp) {
			value = critnib_remove(data->stream->ready_timestamps, committed_timestamp);
			if (value) {
				/* Synchronize on remove to make sure that only one thread perform this operation. */
				pmemstream_increase_committed_timestamp(data->stream, (uint64_t)value);
			}
			committed_timestamp += (uint64_t)value;
			value = critnib_get(data->stream->ready_timestamps, committed_timestamp);
		}
	}
	...
	if (committed_timestamp == data->first_timestamp) {
	...
	} else if (data->last_timestamp == data->processing_timestamp && data->timestamp > data->last_timestamp) {
		...
		if (ret) // XXX: in that case, we can't acquire next batch
			return FUTURE_STATE_RUNNING;
	}
