...
void span_create_entry(struct pmemstream *stream, uint64_t offset, const void *data, size_t data_size, size_t popcount,
		       int flags)
{
	span_bytes *span = (span_bytes *)span_offset_to_span_ptr((const struct pmemstream *)stream, offset);
	assert((data_size & SPAN_TYPE_MASK) == 0);
...
{
	const span_bytes *span = span_offset_to_span_ptr((const struct pmemstream *)stream, offset);
...
{
	span_bytes *span = (span_bytes *)span_offset_to_span_ptr((struct pmemstream *)stream, offset);
	struct span_runtime srt;
