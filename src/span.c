...
span_bytes *span_offset_to_span_ptr(const struct pmemstream *stream, uint64_t offset)
{
...
void span_create_empty(const struct pmemstream *stream, uint64_t offset, size_t data_size)
{
 ...
 */
void span_create_entry(const struct pmemstream *stream, uint64_t offset, const void *data, size_t data_size, size_t popcount,
		       int flags)
{
	span_bytes *span = span_offset_to_span_ptr(stream, offset);
	assert((data_size & SPAN_TYPE_MASK) == 0);
...
void span_create_region(const struct pmemstream *stream, uint64_t offset, size_t size)
{
	span_bytes *span = span_offset_to_span_ptr(stream, offset);
