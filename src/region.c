...
static void region_runtime_initialize_read_ready(struct pmemstream_region_runtime *region_runtime,
					       ...
					       struct pmemstream_entry tail)
{
	if (region_runtime_get_state_acquire(region_runtime) == REGION_RUNTIME_STATE_UNINITIALIZED) {
		...
		if (region_runtime_get_state_acquire(region_runtime) == REGION_RUNTIME_STATE_UNINITIALIZED) {
			region_runtime_initialize_read_ready(region_runtime, tail);
		}
...
static int region_runtime_initialize_for_read(struct pmemstream *stream, struct pmemstream_region region,
					      struct pmemstream_region_runtime *region_runtime)
{
	 ...
	 * region_lock. */
	int ret = entry_iterator_initialize(&iterator, stream, region, &region_runtime_initialize_read_ready);
	if (ret) {
		return ret;
	}
					       ...
					       struct pmemstream_region_runtime *region_runtime)
{
	...
	if (state_locked == REGION_RUNTIME_STATE_UNINITIALIZED) {
		ret = region_runtime_initialize_for_read(stream, region, region_runtime);
