...
/* Asynchronous append. Appends data to the region and mark it as ready for commit.
 *
 * There is no guarantee whether data is visible by iterators or persisted after this call.
 * To commit (and make the data visible to iterators) or persist the data use: pmemstream_async_wait_committed or
 * pmemstream_async_wait_persisted.
...
/* Return committed timestamp. All entries with timestamps less than that timestamp can be treated as committed. */
uint64_t pmemstream_committed_timestamp(struct pmemstream *stream);

/* Return persisted timestamp. All entries with timestamps less than that timestamp can be treated as persisted.
