...
int critnib_insert(struct critnib *c, word key, void *value, int update)
{
	...
	if (!n) {
		atomic_store_release(&c->root, kn);
	...
	if (!n) {
		n = prev;
		atomic_store_release(&n->child[slice_index(key, n->shift)], kn);
...
void *critnib_remove(struct critnib *c, word key)
{
	...
	if (is_leaf(n)) {
		k = to_leaf(n);
		if (k->key == key) {
			atomic_store_release(&c->root, NULL);
			goto del_leaf;
		}
	...
	atomic_store_release(&n->child[slice_index(key, n->shift)], NULL);
	...
	atomic_store_release(n_parent, n->child[ochild]);
