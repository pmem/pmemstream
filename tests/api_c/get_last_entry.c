...
/**
 * get_last_entry - unit test for getting last entry using pmemstream_entry_iterator_next in the loop
 */
...
void test_get_last_entry(char *path)
{
	...
	struct entry_data *e = malloc((uint64_t)n * sizeof(struct entry_data));
	...
	UT_ASSERTeq(last_entry_data->data, e[n - 1].data);
