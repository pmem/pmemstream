...
struct stream {
	...
	std::tuple<int, pmemstream_entry> publish(struct pmemstream_region region, const void *data, size_t size,
						  struct pmemstream_entry reserved_entry,
						  pmemstream_region_runtime *region_runtime = nullptr)
	{
		int ret = pmemstream_publish(c_stream.get(), region, region_runtime, data, size, reserved_entry);
	...
	auto entry_iterator(pmemstream_region region)
	{
		...
		UT_ASSERTeq(pmemstream_entry_iterator_new(&eiter, c_stream.get(), region), 0);
	...
	auto region_iterator()
	{
		...
		UT_ASSERTeq(pmemstream_region_iterator_new(&riter, c_stream.get()), 0);
...
struct pmemstream_helpers_type {
	pmemstream_helpers_type(pmem::stream &s) : s(s)
	{
	}
	...
	void append(struct pmemstream_region region, const std::vector<std::string> &data)
	{
		for (const auto &e : data) {
			auto [ret, entry] = s.append(region, e, region_runtime[region.offset]);
			UT_ASSERTeq(ret, 0);
		}
	}
	...
	{
		auto [ret, region] = s.region_allocate(region_size);
		UT_ASSERTeq(ret, 0);
		/* region_size is aligned up to block_size, on allocation, so it may be bigger than expected */
		UT_ASSERT(s.region_size(region) >= region_size);

		if (call_region_runtime_initialize) {
			auto [ret, runtime] = s.region_runtime_initialize(region);
	...
	void reserve_and_publish(struct pmemstream_region region, const std::vector<std::string> &data)
	{
		for (const auto &d : data) {
			...
			auto [ret, reserved_entry, reserved_data] =
				s.reserve(region, d.size(), region_runtime[region.offset]);
			UT_ASSERTeq(ret, 0);

			/* write into the reserved space and publish (persist) it */
			memcpy(reserved_data, d.data(), d.size());

			auto [ret_p, entry_p] = s.publish(region, reinterpret_cast<const void *>(d.data()), d.size(),
							  reserved_entry, region_runtime[region.offset]);
			UT_ASSERTeq(entry_p.offset, reserved_entry.offset);
			UT_ASSERTeq(ret, 0);
		}
	}
	...
	{
		auto eiter = s.entry_iterator(region);
	...
	std::vector<std::string> get_elements_in_region(struct pmemstream_region region)
	{
		...
		while (pmemstream_entry_iterator_next(eiter.get(), &r, &entry) == 0) {
			...
			result.emplace_back(s.get_entry(entry));
		}
