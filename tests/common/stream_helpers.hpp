...
struct pmemstream_helpers_type {
	pmemstream_helpers_type(pmem::stream &stream) : stream(stream)
	{
	}
	...
	void reserve_and_publish(struct pmemstream_region region, const std::vector<std::string> &data)
	{
		for (const auto &d : data) {
			...
			auto [ret_p, entry_p] = stream.publish(region, reinterpret_cast<const void *>(d.data()),
							       d.size(), reserved_entry, region_runtime[region.offset]);
