...
struct rc_append_command : public pmemstream_command {
	std::map<uint64_t, std::vector<std::string>> data_to_append;

	explicit rc_append_command(const pmemstream_model &m)
	{
		for (auto &region : m.regions) {
			data_to_append[region.first] = *rc::gen::arbitrary<std::vector<std::string>>();
		}
	}
	...
	void apply(pmemstream_model &m) const override
	{
		for (auto &data : data_to_append) {
			...
			region.insert(region.end(), data.second.begin(), data.second.end());
		}
	}
...
struct Arbitrary<pmemstream_empty> {
	...
	{
		return Gen<pmemstream_empty>([&](const Random &random, int size) {
			return shrinkable::just<pmemstream_empty>(pmemstream_empty{});
		});
	}
};
...
struct Arbitrary<pmemstream_with_single_empty_region> {
	...
	{
		return Gen<pmemstream_with_single_empty_region>([&](const Random &random, int size) {
			return shrinkable::just<pmemstream_with_single_empty_region>(
				pmemstream_with_single_empty_region{});
		});
	}
};
...
struct Arbitrary<pmemstream_with_single_init_region> {
	...
	{
		return gen::construct<pmemstream_with_single_init_region>(gen::arbitrary<std::vector<std::string>>());
	}
};
