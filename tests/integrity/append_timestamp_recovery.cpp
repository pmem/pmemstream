...
static void test(char mode)
{
	...
	if (mode == 'a') {
		...
		std::cout << "CHECK after get_region" << std::endl;
		...
		std::cout << "CHECK before append" << std::endl;
		/* timestamp should be greater than elements count by 1 */
		...
		UT_ASSERTeq(cnt + 1, committed_timestamp);
		UT_ASSERTeq(cnt + 1, persisted_timestamp);
