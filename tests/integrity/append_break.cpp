...
static void test(int argc, char *argv[])
{
	...
	if (argv[1][0] == 'a') {
		...
		pmemstream_test_base s(path, get_test_config().block_size, get_test_config().stream_size);
		s.helpers.initialize_single_region(TEST_DEFAULT_REGION_SIZE, init_data);

	} else if (argv[1][0] == 'b') {
		/* break in the middle of an append */

		pmemstream_test_base s(path, get_test_config().block_size, 0, false);
...
int main(int argc, char *argv[])
{
	...
	return run_test(test_config, [&] { test(argc, argv); });
}
