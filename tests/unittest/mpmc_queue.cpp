...
struct mpmc_sut {
	...
	decltype(make_mpmc_queue(0ULL, 0ULL)) queue;
};
...
struct acquire_command : public mpmc_command {
	...
	size_t size = *rc::gen::arbitrary<size_t>();
...
int main(int argc, char *argv[])
{
	return run_test([&] {
		...
		ret += rc::check("verify acquire/produce sequence in single thread", []() {
			mpmc_model m;
			mpmc_sut q;
			rc::state::check(m, q, rc::state::gen::execOneOfWithArgs<acquire_command, produce_command>());
		});
	});
}
