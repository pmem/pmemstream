...
struct singly_linked_list_insert_command : singly_linked_list_command {
	...
	{
		/* RC commands live for the whole TC lifetime (unless they don't fulfill preconditions),
		 * so it should be safe to cleanup in command's dtor. */
		delete n;
	}
};
...
struct rc_insert_head_command : singly_linked_list_insert_command {
	...
	{
		SLIST_INSERT_HEAD(struct node, &s.runtime, &s.sut, (uint64_t)n - TEST_SLIST_DATA_BASE, next);
	}
};
...
struct rc_insert_tail_command : singly_linked_list_insert_command {
	...
	void run(const singly_linked_list_model &m, singly_linked_list_test &s) const override
	{
		SLIST_INSERT_TAIL(struct node, &s.runtime, &s.sut, (uint64_t)n - TEST_SLIST_DATA_BASE, next);
	}
};
