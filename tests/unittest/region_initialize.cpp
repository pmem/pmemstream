	...
	return run_test([&] {
		{
			...
			UT_ASSERTeq(region_runtime_is_initialized(region_runtime), false);
			parallel_xexec(concurrency, [&](size_t id, std::function<void(void)> syncthreads) {
				if (id % 2 == 0) {
					...
					region_runtime_initialize(region_runtime, entry);

					UT_ASSERTeq(region_runtime_is_initialized(region_runtime), true);
				} else {
					syncthreads();

					for (size_t i = 0; i < num_repeats; i++) {
						auto initialized = region_runtime_is_initialized(region_runtime);
						if (initialized) {
