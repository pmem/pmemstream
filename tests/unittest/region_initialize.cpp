	...
	return run_test([&] {
		{
			...
			UT_ASSERTeq(region_runtime_get_state(region_runtime), REGION_RUNTIME_STATE_UNINITIALIZED);
			parallel_xexec(concurrency, [&](size_t id, std::function<void(void)> syncthreads) {
				if (id % 2 == 0) {
					...
					UT_ASSERTeq(region_runtime_get_state(region_runtime),
						    REGION_RUNTIME_STATE_DIRTY);
				} else {
					syncthreads();

					for (size_t i = 0; i < num_repeats; i++) {
						auto initialized = region_runtime_get_state(region_runtime) !=
			...
			UT_ASSERTeq(region_runtime_get_state(region_runtime), REGION_RUNTIME_STATE_DIRTY);
