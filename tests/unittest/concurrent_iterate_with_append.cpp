...
int main(int argc, char *argv[])
{
	...
	return run_test([&] {
		...
		std::string rapidcheck_config =
			"RC_PARAMS=\"noshrink=1\" max_size=\"" + std::to_string(max_entry_size) + "\"";
		int r = putenv(rapidcheck_config.data());
		UT_ASSERT(r == 0);
		...
		ret += rc::check(
			...
			[&](const std::vector<std::string> &data, const std::vector<std::string> &extra_data,
				...
				auto stream =
					make_pmemstream(path, TEST_DEFAULT_BLOCK_SIZE, TEST_DEFAULT_STREAM_SIZE, false);
