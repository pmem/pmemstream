			  ...
			  const std::vector<consumer_state> &consumed_data)
{
	...
	for (auto r : all_consumed_ranges) {
		UT_ASSERTeq(r.offset, offset);
		offset += r.size;
	}

	UT_ASSERTeq(offset, sum_sizes_to_produce);
}
...
{
	unsigned num_consumers = *rc::gen::inRange<unsigned>(1, max_concurrency - sizes_to_produce.size());
	mpmc_test_generic(sizes_to_produce, num_consumers);
}
