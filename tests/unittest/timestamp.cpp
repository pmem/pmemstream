...
void multithreaded_synchronous_append(pmemstream_test_base &stream, std::vector<pmemstream_region> regions,
...
int main(int argc, char *argv[])
{
	...
	return run_test(test_config, [&] {
		...
		ret += rc::check(
			...
			[&](pmemstream_with_multi_empty_regions<true, true> &&stream, const std::vector<std::string> &data) {
				RC_PRE(data.size() > 0);
